...
object ContactTransformOIHInstance : TransformOIH<EitherPartialOf<HandlerException>, Contact, OIHContact> {
  ...
  override fun OIHContact.transformToFileee(): Kind<EitherPartialOf<HandlerException>, Contact> = when (this) {
    ...
    is OIHContact.OIHPerson -> firstName.map { first -> lastName.fold({ first }, { "$first $it" }) }.orElse { lastName }
...
internal fun findFileeeRecordId(records: Nel<OIHApplicationRecord>): Option<UUID> = records.all.find { it.applicationUid == FileeeApplicationId }.toOption()
  .flatMap { it.recordUid }

internal fun transformAddressToFileee(addr: OIHAddress): Address = Address(
...
internal fun isBrandingInfo(contactData: OIHContactData): Boolean = when (contactData.type) {
...
internal fun transformContactDataToFileee(contactData: OIHContactData): ContactData = ContactData(
  value = contactData.value,
  type = contactData.type.toUpperCase(),
  description = contactData.description
)

internal fun brandingInfoFromContactData(contactData: List<OIHContactData>): Option<BrandingInfo> = when (contactData.any { isBrandingInfo(it) }) {
  true -> BrandingInfo(
...
internal fun transformAddressToOIH(addr: Address): OIHAddress = OIHAddress(
  street = addr.street,
  streetNumber = none(),
  description = none(),
  primaryContact = none(),
...
internal fun transformContactDataToOIH(contData: ContactData): OIHContactData = OIHContactData(
  value = contData.value,
  type = contData.type.toLowerCase(),
  description = contData.description
)

internal fun contactDataFromBrandingInfo(brand: BrandingInfo): List<OIHContactData> = brand.let { brandingInfo ->
